...
// 边缘棋子（任一方向越界）不参与判定
// 返回首个被自包围的己方内部子坐标 {r,c}，否则 null（高亮用）
function findSelfSurroundStone(board, player) {
...
function checkSelfSurroundLoss(board, player) {
  return !!findSelfSurroundStone(board, player);
}
    ...
    for (i = 0; i < hRuns[r].length; i++) {
      for (j = 0; j < hRuns[r + 1].length; j++) {
        ...
        if (rangesOverlap(ri.c1, ri.c2, rj.c1, rj.c2)) {
          ...
          if (overlapLen >= shorterLen) return { direction: 'h', cells: ri.cells.concat(rj.cells) };
        }
      }
    }
  }
    ...
    for (i = 0; i < vRuns[c].length; i++) {
      for (j = 0; j < vRuns[c + 1].length; j++) {
        ...
        if (rangesOverlap(vi.r1, vi.r2, vj.r1, vj.r2)) {
          ...
          if (overlapLen >= shorterLen) return { direction: 'v', cells: vi.cells.concat(vj.cells) };
        }
      }
    }
  }
  ...
  for (let idx = 0; idx < d1Runs.length - 2; idx++) {
    for (i = 0; i < d1Runs[idx].length; i++) {
      for (j = 0; j < d1Runs[idx + 2].length; j++) {
        if (chainDiagMatch(d1Runs[idx][i], d1Runs[idx + 2][j], 1, -1)) return { direction: 'd1', cells: d1Runs[idx][i].cells.concat(d1Runs[idx + 2][j].cells) };
      }
    }
  }
  ...
  for (let idx = 0; idx < d2Runs.length - 2; idx++) {
    for (i = 0; i < d2Runs[idx].length; i++) {
      for (j = 0; j < d2Runs[idx + 2].length; j++) {
        if (chainDiagMatch(d2Runs[idx][i], d2Runs[idx + 2][j], 1, 1)) return { direction: 'd2', cells: d2Runs[idx][i].cells.concat(d2Runs[idx + 2][j].cells) };
      }
    }
  }
...
function evaluateMove(board, lastR, lastC, player) {
  const size = board.length;
  // 2: 规则三（自包围判负）— 落子方自包围 → 落子方负
  const selfPos = findSelfSurroundStone(board, player);
  if (selfPos) {
    const stones = [];
    for (let i = 0; i < ALL8.length; i++) {
      const nr = selfPos.r + ALL8[i][0], nc = selfPos.c + ALL8[i][1];
      if (inBounds(nr, nc, size)) stones.push({ r: nr, c: nc });
    }
    return {
      gameOver: true, winner: opponent(player), reason: '八方全占（自包围）', rule: 3,
      winTarget: selfPos, winStones: stones
    };
  }
  // 3: 规则四（连续两排判负）— 落子方双排 → 落子方负
  ...
  if (rowResult) {
    ...
    const target = cells.length ? cells[Math.floor((cells.length - 1) / 2)] : null;
    return {
      gameOver: true, winner: opponent(player), reason: dirText + '连续超过三颗', rule: 4,
      winTarget: target, winStones: cells
    };
  }
